def pdf_to_html(text_attributes_list):
    ...
    while i < n:
        ...
        if size == 16 and font in ["ITCKabelStd-BoldRO"] and color == 10887518:
       ...
       <p class="clear"></p>
            ...
            size in [11.270000457763672, 10.21099853515625] and font == "PTSans-Regular"
        ):
            # Nume autor
            html = f"""
        <h5>{text}</h5>"""
        ...
        elif size == 15 and font == "ITCKabelStd-BoldRO" and color == 8399247:
            html = f"""
        <h2 class="lesson-title">{text}</h2>"""
            ...
            new_text_attributes_list.append(
                [html, size, "PTSans-Regular", color, bkgr_color]
            )
            i += 1

        elif (
            10.5 <= size <= 11.5 or size == 10.800000190734863
        ) and font == "MinionPro-It":
            # Italic MinionPro
            html = f"""<span class="italic">{text}</span>"""
            new_text_attributes_list.append(
                [html, size, "MinionPro-Regular", color, bkgr_color]
            )
            i += 1
            ...
            html = f"""<span class="italic">{text}</span>"""
            new_text_attributes_list.append(
                [html, size, "Roboto-Light", color, bkgr_color]
            )
            i += 1

        elif (
            10 <= size <= 11.6
            and font == "MinionPro-Bold"
            and color == 2236191
            and text.strip() != "•"
        ):
            # Bold MinionPro
            ...
            new_text_attributes_list.append(
                [html, 10.779999732971191, "MinionPro-Regular", color, bkgr_color]
            )
            i += 1
            ...
            new_text_attributes_list.append(
                [html, size, "PTSans-Regular", 2236191, bkgr_color]
            )
            i += 1
        elif (
            size == 14.0
            ...
            and text != "Portofoliu"
            and text != "Important"
            and text != "Organizarea portofoliului"
        ):
            # nume paragraf
            html = f"""
        <h2 class="lesson-title">{text}</h2>"""
        ...
        elif text == "Experiment" and size == 14.0 and font == "ITCKabelStd-BoldRO":
            # Experiment
            html = f"""
        ...
        """
            new_text_attributes_list.append([html, 0, "0", 0, (257, 0, 0)])
            i += 1
        elif size == 11.5 and font == "MinionPro-Bold" and color == 32197:
            # text albastru
            ...
            new_text_attributes_list.append(
                [html, size, "LMSans10-Regular", 2236191, bkgr_color]
            )
            i += 1
        elif (
            font == "LMSans10-Bold" and size == 10.909099578857422 and color == 3027090
        ):
            # model
            ...
            new_text_attributes_list.append(
                [html, size, "MinionPro-Regular", 2236191, bkgr_color]
            )
            i += 1
        ...
        <h4>{text}</h4>"""
            ...
            bkgr_color in [(236, 0, 140), (236, 0, 139)]
            or text == "Organizarea portofoliului"
        ) and text not in ["Important"]:
            # portofoliu
            html = f"""
        <h3 class="portofoliu">{text}</h3>"""
            ...
            text.strip() == "•"
            and size in [11.5, 11.030412673950195, 11.557356834411621]
            and font in ["MinionPro-Bold", "SymbolMT", "ITCKabelStd-BoldRO"]
        ):
            new_text_attributes_list.append(
                [text, size, "MinionPro-Regular", color, bkgr_color]
            )
            i += 1
        elif i < n - 1:
