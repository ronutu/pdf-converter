...
def combine_text(text_attributes_list):
    ...
    for element in text_attributes_list:
        ...
        if (
            previous_size - 0.6 <= size <= previous_size + 0.6
            and font == previous_font
            and bg_color == previous_bg_color
        ):
    ...
    while i < n:
        ...
        if len(result) == 1:
            ...
            new_text_attributes_list.append(
                [
                    result[0],
                    text_attributes_list[i][1],
                    text_attributes_list[i][2],
                    text_attributes_list[i][3],
...
def combine_patrat_albastru(text_attributes_list):
    ...
    while i < n:
        ...
        if (item[0] == "\x83" and item[2] == "Wingdings-Regular") or (
            item[0] == "�" and item[2] == "Wingdings-Regular"
        ):
            if i + 1 < n:
                list_items.append(text_attributes_list[i + 1])
                i += 2
...
def is_exercitiu_pair(current, next_item):
    ...
    return (
        9 <= size <= 10
        and font == "PTSans-Bold"
        and bg_color2 == (255, 255, 255)
        and font2 == "MinionPro-Regular"
    )
...
def combine_exercitiu(text_attributes_list):
    ...
    if (
        ...
        and new_text_attributes_list[-1] != text_attributes_list[-1]
    ):
