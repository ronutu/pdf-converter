...
def bkgr_read(text_attributes_list):
    ...
    n = len(text_attributes_list)
    while i < n:
        if text_attributes_list[i][4] in [
            (255, 253, 232),
            (255, 243, 222),
            (255, 253, 233),
        ]:
...
def bkgr_pers(text_attributes_list):
    ...
    while i < n:
        if (
            text_attributes_list[i][1] == 10.779999732971191
            and text_attributes_list[i][2] == "PTSans-Regular"
            and text_attributes_list[i][4] in [(235, 247, 248), (234, 246, 247)]
        ):
            # bkgr_pers
            html = f"""
                <div class="bkgr-pers">
...
def text_imp(text_attributes_list):
    ...
    while i < len(text_attributes_list):
        if (
            text_attributes_list[i][0].strip() == "Important"
            or text_attributes_list[i][0].strip() == "Concluzii"
        ):
            html = f"""
        <h3 class="important">{text_attributes_list[i][0].strip()}</h3>"""
            new_text_attributes_list.append([html, 0, "0", 0, (257, 0, 0)])
            i += 1
        elif text_attributes_list[i][4] in [
            (258, 0, 0),
            (237, 246, 231),
            (238, 246, 232),
            (236, 240, 231),
...
def text_galben(text_attributes_list):
    ...
    while i < len(text_attributes_list):
        ...
        if bkgr_color in [(254, 243, 229), (255, 244, 230)]:
            ...
            bkgr_color not in [(254, 243, 229), (255, 244, 230)]
            and text_attributes_list[i - 1][4] in [(254, 243, 229), (255, 244, 230)]
        ) or (text_attributes_list[i] == text_attributes_list[-1]):
...
def paragraph(text_attributes_list):
    ...
    while i < n:
        ...
        if (
            size == 11.5
            and font == "MinionPro-Regular"
            and bkgr_color == (255, 255, 255)
        ):
            # paragraph
            html = f"""
            <p>{text_attributes_list[i][0]}</p>
...
def text_mov(text_attributes_list):
    ...
    while i < len(text_attributes_list):
        ...
        if bkgr_color in [(231, 223, 237), (233, 225, 239)]:
            ...
            bkgr_color not in [(231, 223, 237)]
            and text_attributes_list[i - 1][4] in [(231, 223, 237)]
        ) or (text_attributes_list[i] == text_attributes_list[-1]):
...
def text_blue(text_attributes_list):
    ...
    while i < len(text_attributes_list):
        ...
        if bkgr_color in [(224, 234, 247)]:
            ...
            bkgr_color not in [(224, 234, 247)]
            and text_attributes_list[i - 1][4] in [(224, 234, 247)]
        ) or (text_attributes_list[i] == text_attributes_list[-1]):
...
def text_model(text_attributes_list):
    ...
    while i < len(text_attributes_list):
        ...
        if bkgr_color in [(224, 222, 239)]:
            ...
            bkgr_color not in [(224, 222, 239)]
            and text_attributes_list[i - 1][4] in [(224, 222, 239)]
        ) or (text_attributes_list[i] == text_attributes_list[-1]):
...
def titlu_unitate_03mat(text_attributes_list):
    ...
    for element in text_attributes_list:
        ...
        if (
            previous_size == 16.0
            and size == 16
            and previous_color == 27537
            and color == 27537
            and previous_font == "Roboto-Bold"
            and font == "Roboto-Bold"
        ) or (
            previous_size == 18.0
            and size == 18
            and previous_color == 16777215
            and color == 16777215
            and previous_font == "Roboto-Bold"
            and font == "Roboto-Bold"
            and previous_bg_color == (98, 199, 204)
            and bg_color == (98, 199, 204)
        ):
            ...
            if previous_text:
                new_new_text_attributes_list.append(
                    [
                        previous_text,
                        previous_size,
                        previous_font,
